...
//    }
    ...
    @PostMapping(path = "/update_customer_team_admin")
    public ResponseEntity<Object> updateCustomerTeam(HttpServletRequest request) {
        ...
        ArrayList<String> errorFields = baseRequest.verifyRequiredParams("match_unique_id", "customer_team_id",
                "player_json","customer_id","user_id");
        if (errorFields.size() > 0) {
            return echoRespose(baseRequest.generateRequiredParamsResponse(errorFields), HttpStatus.OK);
        }

        BaseResponse match = cronService.updateCustomerTeam(baseRequest);
        return echoRespose(match, HttpStatus.OK);
    }
    ...
    @GetMapping(path = "/match_scorecard_cron/{match_unique_id}")
    public ResponseEntity<Object> matchScorecardCron(HttpServletRequest request,
                                                     @PathVariable(name = "match_unique_id") int match_unique_id) {

        BaseRequest baseRequest = new BaseRequest(request);
        BaseResponse match = cronService.matchScorecardCron(match_unique_id);
        return echoRespose(match, HttpStatus.OK);
    }
