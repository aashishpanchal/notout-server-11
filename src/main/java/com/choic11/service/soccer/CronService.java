...
import com.choic11.model.soccer.TblSoccerMatchPlayersStat;
import com.choic11.model.soccer.TblSoccerSeries;
import com.choic11.repository.soccer.CronRepository;
...
public class CronService {
    ...
    public BaseResponse distributeReferralCashbonusCron(int matchUniqueId) {
        ...
        if (distributeReferralCashbonusCron.size() == 0) {
            ...
            TblSoccerMatch matchDataOnly = matchesService.getMatchesRepository().getMatchDataOnly(matchUniqueId);
    ...
    public BaseResponse declareMatchResult(int match_unique_id) {
        ...
        TblSoccerMatch matchDataOnly = matchesService.getMatchesRepository().getMatchData(match_unique_id);
    ...
    public BaseResponse declareSeriesLeaderboardResult(int seriesId) {
        ...
        TblSoccerSeries tblSoccerSeries = cronRepository.getSeriesData(seriesId);

        if (tblSoccerSeries == null) {
            return new BaseResponse(0, true, "No series found.", null);
        }

        if (tblSoccerSeries.isResultDeclared()) {
            return new BaseResponse(0, true, "Result already declared.", null);
        }

        if (tblSoccerSeries.isResultRunning()) {
        ...
        if (updateSeriesProgress > 0) {
            startSeriesLeaderboardResultDeclareForMatch(tblSoccerSeries);
        }
    ...
    public BaseResponse abodentMatch(int match_unique_id) {
        ...
        TblSoccerMatch matchDataOnly = matchesService.getMatchesRepository().getMatchDataOnly(match_unique_id);
            ...
            public void run() {
                try {
                    ...
                    if (players != null && !players.isEmpty()) {
                        ...
                        LinkedHashMap<String, TblSoccerMatchPlayersStat> playersStatsForMatchUniqueId = cronRepository
    ...
    public void startReferralDistributeForMatch(TblSoccerMatch matchDataOnly, HashMap<String, String> referCashbonus) {
    ...
    public void startAffiliateDistributeForMatch(TblSoccerMatch matchDataOnly) {
        if (matchDataOnly == null) {
            return;
        }
    ...
    public void startLeaderboardCreatingForMatch(TblSoccerMatch matchDataOnly) {
        if (matchDataOnly == null) {
            return;
        }
    ...
    public void startAboundantDeclareForMatch(TblSoccerMatch matchDataOnly) {
        if (matchDataOnly == null) {
            return;
        }
    ...
    public void startResultDeclareForMatch(TblSoccerMatch matchDataOnly,
    ...
    public void startResultDeclareForMatchTest(TblSoccerMatch matchDataOnly,
                                               LinkedHashMap<String, Object> modifiedMatchData) {
        if (matchDataOnly == null) {
            return;
        }
    ...
    public void startSeriesLeaderboardResultDeclareForMatch(TblSoccerSeries tblSoccerSeries) {
        if (tblSoccerSeries == null) {
            return;
        }
        ...
        AsyncConfiguration.getSeriesLeaderboardResultExecutor().submit(new Runnable() {
            ...
            public void run() {
                ...
                content = content.replace("{APP_LOGO}", GlobalConstant.WIN_MAIL_LOGO);
                content = content.replace("{SERIES_NAME}",tblSoccerSeries.getName());

                winningTamplate.setContent(content);

                HashMap<String, Object> saveResultForSeries = cronRepository.saveResultForSeriesLeaderboard(tblSoccerSeries,
                        totalTaxPercent, winningTamplate);
        ...
        if (playersForDreamTeam != null && !playersForDreamTeam.isEmpty()) {
            ...
            for (HashMap<String, Object> hashMap : playersForDreamTeam) {
                ...
                if (playingRole != null && !playingRole.trim().isEmpty()) {
                    ...
                    if (playingRole.equals("goalkeeper")) {
                    ...
                    else if (playingRole.equals("defender")) {
                    ...
                    else if (playingRole.equals("midfielder")) {
                    ...
                    else if (playingRole.equals("forward")) {
                        bowlers.add(hashMap);
...
//        }
    ...
    public BaseResponse sendMailCron() {
        ...
        return new BaseResponse(0, false, "Soccer email send cron", emails);
    }
    ...
    public JSONObject generatePointSystems(LinkedHashMap<String, Object> pointSystemForGameTypeId) {
        ...
        if (pointSystemForGameTypeId.containsKey("Goal")) {
            game_type_point.put("Goal",
                    Float.parseFloat(pointSystemForGameTypeId.get("Goal").toString()));
        } else {
            game_type_point.put("Goal", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Assist")) {
            game_type_point.put("Assist",
                    Float.parseFloat(pointSystemForGameTypeId.get("Assist").toString()));
        } else {
            game_type_point.put("Assist", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Shot On Target")) {
            game_type_point.put("Shot_On_Target",
                    Float.parseFloat(pointSystemForGameTypeId.get("Shot On Target").toString()));
        } else {
            game_type_point.put("Shot_On_Target", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Chance Created")) {
            game_type_point.put("Chance_Created",
                    Float.parseFloat(pointSystemForGameTypeId.get("Chance Created").toString()));
        } else {
            game_type_point.put("Chance_Created", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("5 Passes Completed")) {
            game_type_point.put("5_Passes_Completed",
                    Float.parseFloat(pointSystemForGameTypeId.get("5 Passes Completed").toString()));
        } else {
            game_type_point.put("5_Passes_Completed", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Tackle Won")) {
            game_type_point.put("Tackle_Won",
                    Float.parseFloat(pointSystemForGameTypeId.get("Tackle Won").toString()));
        } else {
            game_type_point.put("Tackle_Won", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Interception Won")) {
            game_type_point.put("Interception_Won", Float.parseFloat(pointSystemForGameTypeId.get("Interception Won").toString()));
        } else {
            game_type_point.put("Interception_Won", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Blocked Shot")) {
            game_type_point.put("Blocked_Shot",
                    Float.parseFloat(pointSystemForGameTypeId.get("Blocked Shot").toString()));
        } else {
            game_type_point.put("Blocked_Shot", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Clearance")) {
            game_type_point.put("Clearance",
                    Float.parseFloat(pointSystemForGameTypeId.get("Clearance").toString()));
        } else {
            game_type_point.put("Clearance", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Saves GK")) {
            game_type_point.put("Saves_GK",
                    Float.parseFloat(pointSystemForGameTypeId.get("Saves GK").toString()));
        } else {
            game_type_point.put("Saves_GK", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Penalty Saved GK")) {
            game_type_point.put("Penalty_Saved_GK",
                    Float.parseFloat(pointSystemForGameTypeId.get("Penalty Saved GK").toString()));
        } else {
            game_type_point.put("Penalty_Saved_GK", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Clean Sheet GK Defender")) {
            game_type_point.put("Clean_Sheet_GK_Defender", Float.parseFloat(pointSystemForGameTypeId.get("Clean Sheet GK Defender").toString()));
        } else {
            game_type_point.put("Clean_Sheet_GK_Defender", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("In Starting 11")) {
            game_type_point.put("In_Starting_11", Float.parseFloat(pointSystemForGameTypeId.get("In Starting 11").toString()));
        } else {
            game_type_point.put("In_Starting_11", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Coming on as a Substitute")) {
            game_type_point.put("Coming_on_as_a_Substitute",
                    Float.parseFloat(pointSystemForGameTypeId.get("Coming on as a Substitute").toString()));
        } else {
            game_type_point.put("Coming_on_as_a_Substitute", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Yellow Card")) {
            game_type_point.put("Yellow_Card", Float.parseFloat(pointSystemForGameTypeId.get("Yellow Card").toString()));
        } else {
            game_type_point.put("Yellow_Card", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Red Card")) {
            game_type_point.put("Red_Card",
                    Float.parseFloat(pointSystemForGameTypeId.get("Red Card").toString()));
        } else {
            game_type_point.put("Red_Card", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Own goal")) {
            game_type_point.put("Own_goal", Float.parseFloat(pointSystemForGameTypeId.get("Own goal").toString()));
        } else {
            game_type_point.put("Own_goal", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Goals Conceded Gk Defender")) {
            game_type_point.put("Goals_Conceded_Gk_Defender",
                    Float.parseFloat(pointSystemForGameTypeId.get("Goals Conceded Gk Defender").toString()));
        } else {
            game_type_point.put("Goals_Conceded_Gk_Defender", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Penalty Missed")) {
            game_type_point.put("Penalty_Missed",
                    Float.parseFloat(pointSystemForGameTypeId.get("Penalty Missed").toString()));
        } else {
            game_type_point.put("Penalty_Missed", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Goal GK Defender")) {
            game_type_point.put("Goal_GK_Defender",
                    Float.parseFloat(pointSystemForGameTypeId.get("Goal GK Defender").toString()));
        } else {
            game_type_point.put("Goal_GK_Defender", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Goal MidFielder")) {
            game_type_point.put("Goal_MidFielder",
                    Float.parseFloat(pointSystemForGameTypeId.get("Goal MidFielder").toString()));
        } else {
