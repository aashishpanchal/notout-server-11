...
import com.choic11.model.cricket.TblCricketMatchPlayersStat;
import com.choic11.model.cricket.TblCricketSeries;
...
public class CronService {
    ...
    public BaseResponse matchScorecardCron(int match_unique_id) {
        LinkedHashMap<Integer, Object> matchScorecardCron = cronRepository.matchScorecardCron(match_unique_id);

        if (matchScorecardCron.isEmpty()) {
            return new BaseResponse(0, false, "No match found.", null);
        } else {
            for (Object data : matchScorecardCron.values()) {
                LinkedHashMap<String, Object> match = (LinkedHashMap<String, Object>) data;
                getAndUpdateMatchScorecard(match);
            }
            return new BaseResponse(0, false, "Matches lineup updated", matchScorecardCron);
        }
    ...
    public BaseResponse distributeReferralCashbonusCron(int matchUniqueId) {
        ...
        if (distributeReferralCashbonusCron.size() == 0) {
            ...
            TblCricketMatch matchDataOnly = matchesService.getMatchesRepository().getMatchDataOnly(matchUniqueId);
    ...
    public BaseResponse declareMatchResult(int match_unique_id) {
        ...
        TblCricketMatch matchDataOnly = matchesService.getMatchesRepository().getMatchData(match_unique_id);
    ...
    public BaseResponse declareSeriesLeaderboardResult(int seriesId) {
        ...
        TblCricketSeries tblCricketSeries = cronRepository.getSeriesData(seriesId);

        if (tblCricketSeries == null) {
            return new BaseResponse(0, true, "No series found.", null);
        }

        if (tblCricketSeries.isResultDeclared()) {
            return new BaseResponse(0, true, "Result already declared.", null);
        }

        if (tblCricketSeries.isResultRunning()) {
        ...
        if (updateSeriesProgress > 0) {
            startSeriesLeaderboardResultDeclareForMatch(tblCricketSeries);
        }
    ...
    public BaseResponse abodentMatch(int match_unique_id) {
        ...
        TblCricketMatch matchDataOnly = matchesService.getMatchesRepository().getMatchDataOnly(match_unique_id);
    ...
    public void getAndUpdateMatchScorecard(LinkedHashMap<String, Object> match) {
        if (match == null) {
            return;
        }

        AsyncConfiguration.getScoreBoardexecutor().submit(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    JSONObject matchScoreCard = entitySportsService.getMatchScoreCard(null, String.valueOf(matchUniqueId), match);

                    if (matchScoreCard != null) {

                        boolean updateMatchScoreCard = cronRepository.updateMatchScoreCard(matchUniqueId, matchScoreCard);

                        message = updateMatchScoreCard ? matchUniqueId + " SUCCESS" : matchUniqueId + " FAILED";

                        if (!updateMatchScoreCard) {
                            Util.printLog2(CronService.class.getSimpleName(), message);
                        }
                    } else {
                        message = matchUniqueId + " NO_SCORECARD";
                        Util.printLog2(CronService.class.getSimpleName(), message);
                    }
    ...
    public void startAbondantContestForMatchTest(int matchUniqueId) {
            ...
            public void run() {
                try {
                    ...
                    if (players != null && !players.isEmpty()) {
                        ...
                        LinkedHashMap<String, TblCricketMatchPlayersStat> playersStatsForMatchUniqueId = cronRepository
    ...
    public void startReferralDistributeForMatch(TblCricketMatch matchDataOnly, HashMap<String, String> referCashbonus) {
    ...
    public void startAffiliateDistributeForMatch(TblCricketMatch matchDataOnly) {
        if (matchDataOnly == null) {
            return;
        }
    ...
    public void startLeaderboardCreatingForMatch(TblCricketMatch matchDataOnly) {
        if (matchDataOnly == null) {
            return;
        }
    ...
    public void startAboundantDeclareForMatch(TblCricketMatch matchDataOnly) {
        if (matchDataOnly == null) {
            return;
        }
    ...
    public void startResultDeclareForMatch(TblCricketMatch matchDataOnly,
    ...
    public void startResultDeclareForMatchTest(TblCricketMatch matchDataOnly,
                                               LinkedHashMap<String, Object> modifiedMatchData) {
        if (matchDataOnly == null) {
            return;
        }
    ...
    public void startSeriesLeaderboardResultDeclareForMatch(TblCricketSeries tblCricketSeries) {
        if (tblCricketSeries == null) {
            return;
        }
        ...
        AsyncConfiguration.getSeriesLeaderboardResultExecutor().submit(new Runnable() {
            ...
            public void run() {
                ...
                content = content.replace("{APP_LOGO}", GlobalConstant.WIN_MAIL_LOGO);
                content = content.replace("{SERIES_NAME}",tblCricketSeries.getName());

                winningTamplate.setContent(content);

                HashMap<String, Object> saveResultForSeries = cronRepository.saveResultForSeriesLeaderboard(tblCricketSeries,
                        totalTaxPercent, winningTamplate);
        ...
        if (playersForDreamTeam != null && !playersForDreamTeam.isEmpty()) {
            ...
            for (HashMap<String, Object> hashMap : playersForDreamTeam) {
                ...
                if (playingRole != null && !playingRole.trim().isEmpty()) {
                    ...
                    if (playingRole.equals("wicketkeeper")) {
                    ...
                    } else if (playingRole.equals("batsman")) {
                    ...
                    } else if (playingRole.equals("allrounder")) {
                    ...
                    } else if (playingRole.equals("bowler")) {
            ...
            if (!dream_team_globle.isEmpty()) {
                ...
                while (dream_team_globle.size() > 11) {
...
//        }
    ...
    public BaseResponse sendMailCron() {
        ...
        return new BaseResponse(0, false, "Cricket email send cron", emails);
    }
    ...
    public JSONObject generatePointSystems(LinkedHashMap<String, Object> pointSystemForGameTypeId) {
        ...
        if (pointSystemForGameTypeId.containsKey("Being Part Of Eleven")) {
            game_type_point.put("Being_Part_Of_Eleven",
                    Float.parseFloat(pointSystemForGameTypeId.get("Being Part Of Eleven").toString()));
        } else {
            game_type_point.put("Being_Part_Of_Eleven", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Every Six Hit")) {
            game_type_point.put("Every_Six_Hit",
                    Float.parseFloat(pointSystemForGameTypeId.get("Every Six Hit").toString()));
        } else {
            game_type_point.put("Every_Six_Hit", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Every Boundary Hit")) {
            game_type_point.put("Every_Boundary_Hit",
                    Float.parseFloat(pointSystemForGameTypeId.get("Every Boundary Hit").toString()));
        } else {
            game_type_point.put("Every_Boundary_Hit", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Every Run Scored")) {
            game_type_point.put("Every_Run_Scored",
                    Float.parseFloat(pointSystemForGameTypeId.get("Every Run Scored").toString()));
        } else {
            game_type_point.put("Every_Run_Scored", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Half Century")) {
            game_type_point.put("Half_Century",
                    Float.parseFloat(pointSystemForGameTypeId.get("Half Century").toString()));
        } else {
            game_type_point.put("Half_Century", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Dismiss For A Duck")) {
            game_type_point.put("Dismiss_For_A_Duck",
                    Float.parseFloat(pointSystemForGameTypeId.get("Dismiss For A Duck").toString()));
        } else {
            game_type_point.put("Dismiss_For_A_Duck", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Wicket")) {
            game_type_point.put("Wicket", Float.parseFloat(pointSystemForGameTypeId.get("Wicket").toString()));
        } else {
            game_type_point.put("Wicket", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Maiden Over")) {
            game_type_point.put("Maiden_Over",
                    Float.parseFloat(pointSystemForGameTypeId.get("Maiden Over").toString()));
        } else {
            game_type_point.put("Maiden_Over", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Five Wicket")) {
            game_type_point.put("Five_Wicket",
                    Float.parseFloat(pointSystemForGameTypeId.get("Five Wicket").toString()));
        } else {
            game_type_point.put("Five_Wicket", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Four Wicket")) {
            game_type_point.put("Four_Wicket",
                    Float.parseFloat(pointSystemForGameTypeId.get("Four Wicket").toString()));
        } else {
            game_type_point.put("Four_Wicket", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Three Wicket")) {
            game_type_point.put("Three_Wicket",
                    Float.parseFloat(pointSystemForGameTypeId.get("Three Wicket").toString()));
        } else {
            game_type_point.put("Three_Wicket", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Two Wicket")) {
            game_type_point.put("Two_Wicket", Float.parseFloat(pointSystemForGameTypeId.get("Two Wicket").toString()));
        } else {
            game_type_point.put("Two_Wicket", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Catch")) {
            game_type_point.put("Catch", Float.parseFloat(pointSystemForGameTypeId.get("Catch").toString()));
        } else {
            game_type_point.put("Catch", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Three Catch")) {
            game_type_point.put("Three_Catch", Float.parseFloat(pointSystemForGameTypeId.get("Three Catch").toString()));
        } else {
            game_type_point.put("Three_Catch", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Lbw Bowled")) {
            game_type_point.put("Lbw_Bowled", Float.parseFloat(pointSystemForGameTypeId.get("Lbw Bowled").toString()));
        } else {
            game_type_point.put("Lbw_Bowled", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Catch And Bowled")) {
            game_type_point.put("Catch_And_Bowled",
                    Float.parseFloat(pointSystemForGameTypeId.get("Catch And Bowled").toString()));
        } else {
            game_type_point.put("Catch_And_Bowled", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Stumping")) {
            game_type_point.put("Stumping", Float.parseFloat(pointSystemForGameTypeId.get("Stumping").toString()));
        } else {
            game_type_point.put("Stumping", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Thirty Runs")) {
            game_type_point.put("Thirty_Runs",
                    Float.parseFloat(pointSystemForGameTypeId.get("Thirty Runs").toString()));
        } else {
            game_type_point.put("Thirty_Runs", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Run Out")) {
            game_type_point.put("Run_Out", Float.parseFloat(pointSystemForGameTypeId.get("Run Out").toString()));
        } else {
            game_type_point.put("Run_Out", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Run Out Catcher")) {
            game_type_point.put("Run_Out_Catcher",
                    Float.parseFloat(pointSystemForGameTypeId.get("Run Out Catcher").toString()));
        } else {
            game_type_point.put("Run_Out_Catcher", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Run Out Thrower")) {
            game_type_point.put("Run_Out_Thrower",
                    Float.parseFloat(pointSystemForGameTypeId.get("Run Out Thrower").toString()));
        } else {
            game_type_point.put("Run_Out_Thrower", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Minimum Balls for Strike Rate")) {
            game_type_point.put("Minimum_Balls_for_Strike_Rate",
                    Float.parseFloat(pointSystemForGameTypeId.get("Minimum Balls for Strike Rate").toString()));
        } else {
            game_type_point.put("Minimum_Balls_for_Strike_Rate", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Minimum Overs for Economy Rate")) {
            game_type_point.put("Minimum_Overs_for_Economy_Rate",
                    Float.parseFloat(pointSystemForGameTypeId.get("Minimum Overs for Economy Rate").toString()));
        } else {
            game_type_point.put("Minimum_Overs_for_Economy_Rate", 0f);
        }

        if (pointSystemForGameTypeId.containsKey("Strike Rate")) {
            game_type_point.put("Strike_Rate", new JSONArray(pointSystemForGameTypeId.get("Strike Rate").toString()));
        } else {
            game_type_point.put("Strike_Rate", new JSONArray());
        }

        if (pointSystemForGameTypeId.containsKey("Economy Rate")) {
            game_type_point.put("Economy_Rate", new JSONArray(pointSystemForGameTypeId.get("Economy Rate").toString()));
        } else {
    ...
    public BaseResponse updateCustomerTeam(BaseRequest baseRequest) {
        ...
        if (customerTeamId <= 0) {
            return new BaseResponse(0, true, "customer_team_id required", null);
        }

        baseRequest.authUserId = customerId;
        baseRequest.setParam("isUpdateAdmin","1");
