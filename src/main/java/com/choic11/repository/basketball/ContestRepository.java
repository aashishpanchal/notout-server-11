...
@Repository("BasketballContestRepository")
public class ContestRepository {
    ...
    public List<HashMap<String, Object>> getMatchContest(int matchId, int matchUniqueId, Integer UserId) {
        ...
        String hqln = "SELECT "
                ...
                + "FROM  TblBasketballContestMatch tccf "
                + "LEFT JOIN TblBasketballCustomerContestTeams tccct ON (tccf.id=tccct.matchContestId AND tccct.customerId=:customerId) "
                + "LEFT JOIN TblBasketballContestCategory tccc ON (tccc.id=tccf.categoryId ) "
                + "LEFT JOIN TblSetting ts ON(ts.key='DISCOUNTED_IMAGE') "
                + "LEFT JOIN TblBasketballCustomerContestFav tcccf ON tcccf.customerId=:customerId AND tcccf.contestId=tccf.contestId "
    ...
    public List<HashMap<String, Object>> getCustomerMatchContest(int matchId, int matchUniqueId, Integer UserId) {
        ...
        String hqln1 = "SELECT " + "tccct.matchContestId as matchContestId from TblBasketballCustomerContestTeams tccct "
                + "LEFT JOIN TblBasketballContestMatch tccm on tccm.id=tccct.matchContestId"
    ...
    public LinkedHashMap<Integer, Object> getCustomerContestTeams(int customerId, List<Integer> matchContestIds) {
        String hqln = "SELECT  "
                ...
                + "tcct.name as team_name " + "FROM  TblBasketballCustomerContest tccc "
    ...
    public List<HashMap<String, Object>> getMatchCategoryContest(int matchId, int matchUniqueId, Integer UserId,
        ...
        String hqln = "SELECT  "
                ...
                + "(CASE WHEN tcccf.id is null THEN 'N' ELSE 'Y' END) as isFavorite "
                + "FROM  TblBasketballContestMatch tccf " + "LEFT JOIN TblSetting ts ON(ts.key='DISCOUNTED_IMAGE') "
                + "LEFT JOIN TblBasketballCustomerContestTeams tccct ON (tccf.id=tccct.matchContestId AND tccct.customerId=:customerId) "
                + " LEFT JOIN TblBasketballContestCategory tccc ON (tccf.categoryId=tccc.id ) "
                + "LEFT JOIN TblBasketballCustomerContestFav tcccf ON tcccf.customerId=:customerId AND tcccf.contestId=tccf.contestId "
    ...
    public HashMap<String, Object> getContestWinnerBreakup(int contestId) {
        String hqln = "SELECT id as id, contestJson as contest_json, gadgetDisclaimer as gadget_disclaimer from TblBasketballContestMatch where status='A' AND isDeleted='N' AND id=:id";
    ...
    public HashMap<String, Object> getMatchContestShareDetail(int matchContestId) {
        String hqln = "SELECT tccm.slug as slug, tccm.isBeatTheExpert as is_beat_the_expert, tcm.image as image,tcm.name as name from TblBasketballContestMatch tccm LEFT JOIN TblBasketballMatch tcm ON (tccm.matchId=tcm.id) where (tccm.id=:matchContestId) AND tccm.status='A' AND tccm.isDeleted='N' AND tcm.status='A' AND tcm.isDeleted='N'";
    ...
    public HashMap<String, Object> getMatchContestPdf(int matchContestId) {
        ...
        String hqln = "SELECT COALESCE(pdf,'') as contest_pdf from TblBasketballContestMatch where id=:id";
    ...
    public HashMap<String, Object> getBeatTheExpertTeamId(int matchContestId) {
        String hqln = "SELECT teamId as team_id from TblBasketballContestMatch where id=:id";
        Query queryy = getSession().createQuery(hqln).setParameter("id", matchContestId);
        List<HashMap<String, Object>> result = (List<HashMap<String, Object>>) queryy
                .setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
    ...
    public int updateBeatTheExpertTeam(int matchContestId, int customerTeamId) {
        String hqln = "UPDATE TblBasketballContestMatch tccm set tccm.teamId=:teamId where tccm.id=:id";
    ...
    public List<HashMap<String, Object>> getContestTeams(int userId, int matchUniqueId, int matchContestId, int pageNo,
                                                         TblBasketballMatch matchDataOnly, int beatExpertTeamId) {
        ...
        if (pageNo == 1) {
            ...
            String hql = "SELECT "
                    ...
                    + "FROM TblBasketballCustomerContest tccc " + "LEFT JOIN TblCustomer tc ON tccc.customerId=tc.id "
                    + "LEFT JOIN TblBasketballCustomerTeam tcct ON tcct.id=tccc.customerTeamId "
    ...
    public long getMatchCustomerContestCountByMatchUniqueId(Integer UserId, int matchUniqueId) {
        ...
        String hqln = "SELECT " + "COUNT (DISTINCT tccc.matchContestId ) as id  "
                ...
                + "RIGHT JOIN TblBasketballContestMatch tccm ON (tccm.id=tccc.matchContestId) " + "WHERE "
    ...
    public int getContestTeamsCount(int matchUniqueId, int matchContestId) {
        ...
        String hqln = "SELECT " + "count(id) as id " + "from TblBasketballCustomerContest tccc " + "where "
    ...
    public HashMap<String, Object> getBeatExpertAdminTeam(int matchUniqueId, int matchContestId) {
        ...
        String hqln = "SELECT " + "tcccat.image as cat_image, "
                ...
                + "from TblBasketballCustomerContest tccc " + "LEFT JOIN TblCustomer tc ON(tccc.customerId=tc.id) "
                + "LEFT JOIN TblBasketballCustomerTeam tcct ON(tcct.id=tccc.customerTeamId) "
                + "LEFT JOIN TblBasketballContestMatch tccm ON(tccm.id=:id) "
                + "LEFT JOIN TblBasketballContestCategory tcccat ON(tcccat.id=tccm.categoryId) " + "where "
    ...
    public int ifTeamAlreadyJoinInContest(Integer UserId, Integer[] customerTeamIds, int matchContestId) {
        ...
        String hqln = "SELECT " + "id " + "FROM TblBasketballCustomerContest " + "WHERE "
    ...
    public HashMap<String, Object> contestDetailForJoin(Integer UserId, int matchContestId) {
        String hqln = "SELECT "
                ...
                + "LEFT JOIN TblBasketballMatch tcm ON(tcm.id=tccm.matchId) "
                + "LEFT JOIN TblBasketballContestCategory tccc ON(tccc.id=tccm.categoryId) "
                + "LEFT JOIN TblBasketballCustomerContestTeams tccct ON (tccm.id=tccct.matchContestId AND tccct.customerId=:customerId)  "
    ...
    public HashMap<String, Object> contestDetailMini(int matchContestId) {
        String hqln = "SELECT "
                ...
                + "FROM TblBasketballContestMatch tccm " + "WHERE "
    ...
    public void createDuplicateMatchContest(int matchContestId) {
        ...
        String hqln = "SELECT "
                ...
                + "FROM TblBasketballContestMatch tccm "
                + "LEFT JOIN TblBasketballContestMatch tccf1 ON(tccm.parentId=tccf1.id AND tccf1.status='A' AND tccf1.isDeleted='N' AND tccf1.isDuplicateAllow='Y' AND tccf1.isPrivate='N' AND tccf1.isBeatTheExpert='N' AND tccf1.duplicateCount>tccf1.duplicateCreatedCount) "
                + "where "
        ...
        if (result.size() > 0) {
            ...
            if (isDuplicateAllow.equals("Y") && duplicateCount > duplicateCreatedCount
                ...
                String hql = "INSERT INTO " + "TblBasketballContestMatch "
                        ...
                        + " FROM TblBasketballContestMatch where id=:parentMatchContestId1";
                ...
                String hqllll = "UPDATE " + "TblBasketballContestMatch "
                ...
                if (parentMatchContestId != matchContestId) {
                    String hqll = "UPDATE " + "TblBasketballContestMatch " + "SET isDuplicatedCreated='Y' " + "where "
                            + "id=:matchContestId";
    ...
    public int selectContestTeamCount(int matchContestId) {
        ...
        String hqln = "SELECT " + "totalJoinedTeam as totalJoinedTeam " + "FROM TblBasketballContestMatch " + "WHERE "
                + "id=:matchContestId";
    ...
    public void incrementContestTeamCount(int matchContestId) {
        ...
        String hqln = "UPDATE " + "TblBasketballContestMatch " + "SET totalJoinedTeam=totalJoinedTeam+1 " + "WHERE "
                + "id=:matchContestId";
    ...
    public void updateCustomerContestTeams(Integer customerId, int matchUniqueId, int matchContestId) {
        String hqln = "SELECT " + "tcct.id as id, tcct.name as name " + "FROM TblBasketballCustomerContest tccc "
                + "LEFT JOIN TblBasketballCustomerTeam tcct ON (tcct.id=tccc.customerTeamId) " + "WHERE "
        ...
        hqln = "SELECT " + "tccct.id as id " + "FROM TblBasketballCustomerContestTeams tccct " + "WHERE "
        ...
        if (result.size() > 0) {
            ...
            if (rowCount > 0) {
                hqln = "UPDATE " + "TblBasketballCustomerContestTeams "
                ...
                TblBasketballCustomerContestTeams tblBasketballCustomerContestTeams = new TblBasketballCustomerContestTeams();
                tblBasketballCustomerContestTeams.setMatchContestId(matchContestId);
                tblBasketballCustomerContestTeams.setMatchUniqueId(matchUniqueId);
                tblBasketballCustomerContestTeams.setCustomerId(customerId);
                tblBasketballCustomerContestTeams.setCustomerTeamIds(customerTeamIds);
                tblBasketballCustomerContestTeams.setCustomerTeamNames(customerTeamNames);

                getSession().save(tblBasketballCustomerContestTeams);
    ...
    public void updateCustomerMatchContestInfo(int matchUniqueId,Integer customerId){
        String hqln = "SELECT COUNT(DISTINCT tccc.matchContestId) AS cnt FROM TblBasketballCustomerContest tccc WHERE tccc.matchUniqueId =:matchUniqueId AND tccc.customerId =:customerId";
    ...
    public HashMap<String, Object> getPrivateContestCategory() {
        String hqln = "SELECT "
                ...
                + "FROM " + "TblBasketballContestCategory tccc " + "where " + "tccc.isPrivate='Y' AND tccc.isDeleted='N'";
    ...
    @Transactional
    public TblBasketballContestMatch createPrivateContest(TblBasketballMatch matchDataOnly,
        ...
        try {
            ...
            TblBasketballContestMatch tblBasketballContestMatch = TblBasketballContestMatch.getInstance();

            tblBasketballContestMatch.setActualEntryFees(entryFee);
            tblBasketballContestMatch.setCategoryId((int) categoryData.get("id"));
            tblBasketballContestMatch.setIsCompressionAllow((String) categoryData.get("is_compression_allow"));
            tblBasketballContestMatch.setConfirmWin(categoryData.get("confirm_win").toString());
            tblBasketballContestMatch.setConfirmWinContestPercentage(
                    Float.parseFloat(categoryData.get("confirm_win_contest_percentage").toString()));
            tblBasketballContestMatch.setContestJson(
                    new JSONObject((Map<String, Object>) privateContestBreakupsFromIds.get("contest_json")).toString());
            tblBasketballContestMatch.setCreatedAt(BigInteger.valueOf(time));
            tblBasketballContestMatch.setEntryFees(entryFee);
            tblBasketballContestMatch.setIsPrivate("Y");
            tblBasketballContestMatch.setSeriesId(matchDataOnly.getSeriesId());
            tblBasketballContestMatch.setMatchId(matchDataOnly.getId());
            tblBasketballContestMatch.setMatchUniqueId(matchDataOnly.getUniqueId());
            tblBasketballContestMatch.setPerUserTeamAllowed(perUserTeamAllowed);
            tblBasketballContestMatch.setTotalJoinedTeam(1);
            tblBasketballContestMatch.setTotalPrice(prizePool);
            tblBasketballContestMatch.setTotalTeam(contestSize);
            tblBasketballContestMatch.setUpdatedAt(BigInteger.valueOf(time));
            tblBasketballContestMatch.setUserId(authUserId);

            newSession.save(tblBasketballContestMatch);

            int matchContestId = tblBasketballContestMatch.getId();

            String slug = Util.getAlphaNumericString(12) + matchContestId + "_";

            tblBasketballContestMatch.setSlug(slug);

            String hqln = "UPDATE TblBasketballContestMatch tccm set tccm.slug=:slug where tccm.id=:id";
            ...
            TblBasketballCustomerContest tblBasketballCustomerContest = TblBasketballCustomerContest.getInstance();
            tblBasketballCustomerContest.setCashBonusWallet(usedBonus);
            tblBasketballCustomerContest.setCreated(BigInteger.valueOf(time));
            tblBasketballCustomerContest.setCustomerId(authUserId);
            tblBasketballCustomerContest.setCustomerTeamId(Integer.parseInt(teamId));
            tblBasketballCustomerContest.setDepositeWallet(usedDeposit);
            tblBasketballCustomerContest.setEntryFees(entryFee);
            tblBasketballCustomerContest.setMatchContestId(matchContestId);
            tblBasketballCustomerContest.setSeriesId(matchDataOnly.getSeriesId());
            tblBasketballCustomerContest.setMatchUniqueId(matchDataOnly.getUniqueId());
            tblBasketballCustomerContest.setUpdated(BigInteger.valueOf(time));
            tblBasketballCustomerContest.setWinningWallet(usedWinning);

            newSession.save(tblBasketballCustomerContest);

            int lastUserJoinedRowId = tblBasketballCustomerContest.getId();
    ...
    @Transactional
    public TblBasketballCustomerContest customerJoinContest(TblBasketballMatch matchDataOnly, int authUserId,
                                                         int matchContestId, LinkedHashMap<String, Object> extraData) {
        synchronized (lockObject) {
            int totalTeam = (int) extraData.get("totalTeam");
            String hqlnn = "SELECT " + "totalJoinedTeam as totalJoinedTeam " + "FROM TblBasketballContestMatch " + "WHERE "
                    + "id=:matchContestId";
            ...
            try {
                ...
                tblBasketballCustomerContest.setCreated(BigInteger.valueOf(time));
                tblBasketballCustomerContest.setCustomerId(authUserId);
                tblBasketballCustomerContest.setCustomerTeamId(teamId);
                tblBasketballCustomerContest.setDepositeWallet(usedDeposit);
                tblBasketballCustomerContest.setEntryFees(entryFee);
                tblBasketballCustomerContest.setMatchContestId(matchContestId);
                tblBasketballCustomerContest.setSeriesId(matchDataOnly.getSeriesId());
    ...
    public int customerSwitchTeam(int customerId, int matchUniqueId, int matchContestId, int customerTeamIdOld,
        ...
        String hql = "UPDATE " + "TblBasketballCustomerContest " + "SET customerTeamId=:customerTeamIdNew " + "where "
    ...
    public void saveJoinedContestMail(TblTemplate template, List<HashMap<String, Object>> mailData,
                                      TblBasketballMatch matchDataOnly, String toName, String toEmail) {
        ...
        try {
            ...
            for (HashMap<String, Object> hashMap : mailData) {
                ...
                tblBasketballEmailCron.setToemail(toEmail);
                tblBasketballEmailCron.setToname(toName);
                tblBasketballEmailCron.setCreatedat(BigInteger.valueOf(currentTime));
                tblBasketballEmailCron.setEmailType("JOIN_CONTEST");

                session.save(tblBasketballEmailCron);
            }
        } catch (Exception e) {
        }
    ...
    public BaseResponse updateContestFavorite(Integer UserId, int masterContestId) {
        ...
        String message= "";
        try {
            String hqlQuery = "SELECT tcccf FROM TblBasketballCustomerContestFav tcccf WHERE tcccf.customerId =:customerId AND tcccf.contestId =:contestId";
            ...
            TblBasketballCustomerContestFav tblBasketballCustomerContestFav = (TblBasketballCustomerContestFav) queryObj.uniqueResult();
            if (tblBasketballCustomerContestFav == null) {
                String mysqlQuery1 = "INSERT " + "INTO tbl_basketball_customer_contest_fav "
                ...
                String hqln = "DELETE FROM TblBasketballCustomerContestFav tcccf WHERE tcccf.customerId =:customerId AND tcccf.contestId =:contestId";
