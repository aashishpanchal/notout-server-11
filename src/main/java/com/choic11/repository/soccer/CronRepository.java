...
@Repository("SoccerCronRepository")
public class CronRepository {
    ...
    public LinkedHashMap<Integer, Object> matchProgressCron() {
        ...
        String hqln = "SELECT " + "tcm.uniqueId as uniqueId, tcm.seriesId as seriesId " + "FROM TblSoccerMatch tcm "
        ...
        if (!matchUniqueIdData.isEmpty()) {
            ...
            hqln = "UPDATE TblSoccerMatch tcm " + "SET tcm.matchProgress = 'L' "
    ...
    public int updateIsDisAffiPerOnMatch(Session session, int matchUniqueId, String value) {
        ...
        String hqln = "UPDATE TblSoccerMatch tcm " + "SET tcm.isDisAffiPer = :value "
    ...
    public boolean distributeAffiliatePercentage(LinkedHashMap<String, Object> match) {
        ...
        try {
            ...
            String hqln = "SELECT " + "DISTINCT (tc1.id) as id, tc1.affiliatePercent as affiliatePercent "
                    + "FROM TblSoccerCustomerContest tccc " + "LEFT JOIN TblCustomer tc ON tc.id=tccc.customerId "
            ...
            if (!paramsAffiliate.isEmpty()) {
                String mysqlQuery = "INSERT " + "INTO tbl_soccer_affiliates "
    ...
    public boolean distributeCustomerAffiliatePercentage(LinkedHashMap<String, Object> match) {
        ...
        try {
            ...
            if (!paramsAffiliate.isEmpty()) {
                String mysqlQuery = "INSERT " + "INTO tbl_soccer_customer_affiliates "
                        + "(series_id, match_unique_id, customer_id, affiliate_percent, created_at)" + " VALUES "
                        + paramsAffiliate;
    ...
    public LinkedHashMap<Integer, Object> matchLineupCron(int matchUniqueId) {
        ...
        String hqln = "SELECT "
                ...
                + "FROM TblSoccerMatch tcm "
                + "LEFT JOIN TblSoccerMatchesLineup tcml ON tcml.matchUniqueId=tcm.uniqueId "
    ...
    public LinkedHashMap<Integer, Object> matchScorecardCron(int matchUniqueId) {
        ...
        String hqln = "SELECT " + "tcm.uniqueId as uniqueId,tcm.team1Id as team1Id,tcm.team2Id as team2Id, "
                ...
                + "FROM TblSoccerMatch tcm " + "LEFT JOIN TblSoccerTeam tct_one ON tct_one.id=tcm.team1Id "
                + "LEFT JOIN TblSoccerTeam tct_two ON tct_two.id=tcm.team2Id "
                + "WHERE tcm.status = 'A' AND tcm.isDeleted = 'N' ";
        if (matchUniqueId != 0) {
            hqln += "AND tcm.uniqueId=:uniqueId";
        } else {
    ...
    public LinkedHashMap<Integer, Object> matchAbondantContestCron(int matchUniqueId) {
        ...
        String hqln = "SELECT " + "tcm.uniqueId as uniqueId " + "FROM TblSoccerMatch tcm "
    ...
    public List<HashMap<String, Object>> getPlayerDetailCron() {
        String hqln = "SELECT " + "tcp.uniqueId as uniqueId " + "FROM TblSoccerPlayer tcp " + "WHERE "
    ...
    public HashMap<Integer, String> getPlayersByMatch(int matchUniqueId) {
        ...
        String hqln = "SELECT " + "tcmp.playerUniqueId as playerUniqueId, " + "tcp.name as name "
                ...
                + "LEFT JOIN TblSoccerPlayer tcp ON (tcp.uniqueId=tcmp.playerUniqueId) " + "WHERE "
    ...
    public int updateContestPdfProgress(int matchContestId, String value, String pdfFileName) {
        ...
        String hqln = "UPDATE TblSoccerContestMatch tccm " + "SET tccm.pdfProcess=:value ";
    ...
    public List<HashMap<String, Object>> generateContestPdfCron(int matchContestId) {
        ...
        String hqln = "SELECT "
                ...
                + "tcm.name as matchName " + "FROM TblSoccerContestMatch tccm "
                + "LEFT JOIN TblSoccerMatch tcm on tcm.uniqueId=tccm.matchUniqueId " + "WHERE ";
        if (matchContestId == 0) {
        ...
        for (HashMap<String, Object> hashMap : result) {
            ...
            hqln = "SELECT "
                    ...
                    + "tc.teamName as teamName " + "FROM TblSoccerCustomerContest tccc "
                    + "LEFT JOIN TblSoccerCustomerTeam tct on tct.id=tccc.customerTeamId "
    ...
    public LinkedHashMap<String, Object> getPointSystemForGameTypeId(int gameTypeId) {
        ...
        String hqln = "SELECT " + "tcp.metaKey as metaKey, tcp.metaValue as metaValue " + "FROM TblSoccerPoint tcp "
    ...
    public JSONObject getPlayersForMatchUniqueId(int matchUniqueId) {
        ...
        String hqln = "SELECT " + "tcmp.playerUniqueId as playerUniqueId, tcmp.playingRole as playingRole "
                + "FROM TblSoccerMatchPlayer tcmp " + "WHERE "
    ...
    @Transactional(readOnly = true)
    public LinkedHashMap<String, TblSoccerMatchPlayersStat> getPlayersStatsForMatchUniqueId(int matchUniqueId) {
        ...
        String hqlQuery = "SELECT " +
                ...
                "tsmps.goal as goal," +
                "tsmps.goalValue as goalValue," +
                "tsmps.assist as assist," +
                "tsmps.assistValue as assistValue," +
                "tsmps.shotOnTarget as shotOnTarget," +
                "tsmps.shotOnTargetValue as shotOnTargetValue," +
                "tsmps.chanceCreated as chanceCreated," +
                "tsmps.chanceCreatedValue as chanceCreatedValue," +
                "tsmps.fivePassesCompleted as fivePassesCompleted," +
                "tsmps.fivePasses_Completed_Value as fivePasses_Completed_Value," +
                "tsmps.tackleWon as tackleWon," +
                "tsmps.tackleWonValue as tackleWonValue," +
                "tsmps.interceptionWon as interceptionWon," +
                "tsmps.interceptionWonValue as interceptionWonValue," +
                "tsmps.blockedShot as blockedShot," +
                "tsmps.blockedShotValue as blockedShotValue," +
                "tsmps.clearance as clearance," +
                "tsmps.clearanceValue as clearanceValue," +
                "tsmps.saves as saves," +
                "tsmps.savesValue as savesValue," +
                "tsmps.penaltySaved as penaltySaved," +
                "tsmps.penaltySavedValue as penaltySavedValue," +
                "tsmps.cleanSheet as cleanSheet," +
                "tsmps.cleanSheetValue as cleanSheetValue," +
                "tsmps.inStarting11 as inStarting11," +
                "tsmps.inStarting11Value as inStarting11Value," +
                "tsmps.comingOnAsASubstitute as comingOnAsASubstitute," +
                "tsmps.comingOnAsASubstituteValue as comingOnAsASubstituteValue," +
                "tsmps.yellowCard as yellowCard," +
                "tsmps.yellowCardValue as yellowCardValue," +
                "tsmps.redCard as redCard," +
                "tsmps.redCardValue as redCardValue," +
                "tsmps.ownGoal as ownGoal," +
                "tsmps.ownGoalValue as ownGoalValue," +
                "tsmps.goalsConceded as goalsConceded," +
                "tsmps.goalsConcededValue as goalsConcededValue," +
                "tsmps.penaltyMissed as penaltyMissed," +
                "tsmps.penaltyMissedValue as penaltyMissedValue," +
                "tsmps.updated as updated " +
                "FROM TblSoccerMatchPlayersStat tsmps WHERE tsmps.matchUniqueId =:matchUniqueId";
        Query queryObj = getSession().createQuery(hqlQuery);
        queryObj.setParameter("matchUniqueId", matchUniqueId);
        List<TblSoccerMatchPlayersStat> list = queryObj
                .setResultTransformer(new AliasToBeanResultTransformer(TblSoccerMatchPlayersStat.class))
                .getResultList();

//        Criteria createCriteria = getSession().createCriteria(TblSoccerMatchPlayersStat.class);
...
//                .setResultTransformer(new AliasToBeanResultTransformer(TblSoccerMatchPlayersStat.class));
//        List<TblSoccerMatchPlayersStat> list = createCriteria.list();

        LinkedHashMap<String, TblSoccerMatchPlayersStat> data = new LinkedHashMap<String, TblSoccerMatchPlayersStat>();
        if (list != null && list.size() > 0) {
            for (TblSoccerMatchPlayersStat tblSoccerMatchPlayersStat : list) {

                data.put(String.valueOf(tblSoccerMatchPlayersStat.getPlayerUniqueId()), tblSoccerMatchPlayersStat);
            }
        }
    ...
    public boolean updateMatchLineup(int matchUniqueId, List<Integer> players, List<Integer> oldPlayers,
        ...
        try {
            ...
            if (!oldPlayers.isEmpty()) {
                ...
                String hqln = "UPDATE TblSoccerMatchPlayer tcmp " + "SET tcmp.isInPlayingSquad = :isInPlayingSquad "
            ...
            if (!oldSubstitute.isEmpty()) {
                ...
                String hqln = "UPDATE TblSoccerMatchPlayer tcmp "
            ...
            if (needUpdatePlayers || needUpdateSubsitude) {
                ...
                if (playerUniqueIds.isEmpty()) {
                    TblSoccerMatchesLineup tblSoccerMatchesLineup = new TblSoccerMatchesLineup();
                    tblSoccerMatchesLineup.setMatchUniqueId(matchUniqueId);
                    tblSoccerMatchesLineup.setPlayerUniqueIds(newPlayerUniqueIds);
                    tblSoccerMatchesLineup.setSubsituteUniqueIds(newSubsituteUniqueIds);
                    tblSoccerMatchesLineup.setUpdatedAt(BigInteger.valueOf(currentTime));

                    session.save(tblSoccerMatchesLineup);

                    String playingSquadUpdated = "Y";
                    String playingSquadUpdatedMannual = "Y";
                    String hqln = "UPDATE TblSoccerMatch tcm "
                    ...
                    String hqln = "UPDATE TblSoccerMatchesLineup tcml "
                    ...
                    if (fromMannual) {
                        String playingSquadUpdatedMannual = "Y";
                        String hqln1 = "UPDATE TblSoccerMatch tcm "
    ...
    public void saveLineupNotificationForCustomers(int matchUniqueId) {
        ...
        String hqln = "SELECT " + "tcm.name as name, " + "tcs.name as seriesName " + "FROM TblSoccerMatch tcm "
                + "LEFT JOIN TblSoccerSeries tcs ON tcs.id=tcm.seriesId "
        ...
        TblSoccerLineupNotiCron tblSoccerLineupNotiCron = new TblSoccerLineupNotiCron();
        tblSoccerLineupNotiCron.setIsSend("N");
        tblSoccerLineupNotiCron.setMessage(data.toString());
        tblSoccerLineupNotiCron.setAlertMessage(alertMessage);
        tblSoccerLineupNotiCron.setUserIds(customerIds);
        tblSoccerLineupNotiCron.setNotiType(noti_type);
        tblSoccerLineupNotiCron.setCreated(BigInteger.valueOf(currentTime));

        session.save(tblSoccerLineupNotiCron);
    ...
    public List<HashMap<String, Object>> sendLineupNotificationCron() {
        ...
        String hqln = "SELECT "
                ...
                + "FROM TblSoccerLineupNotiCron tclnc " + "WHERE tclnc.isSend = 'N'";
    ...
    public List<HashMap<String, Object>> sendMailCron() {
        ...
        String hqln = "SELECT "
                ...
                + "FROM TblSoccerEmailCron tce " + "WHERE tce.isSend = 'N'";
    ...
    public void deleteLineupNotification(int id) {
        ...
        String hqln = "DELETE FROM TblSoccerLineupNotiCron tclnc "
    ...
    public void deleteLineupNotificationMulti(List<Integer> ids) {
        ...
        String hqln = "DELETE FROM TblSoccerLineupNotiCron tblnc "
    ...
    public void deleteEmailCron(int id) {
        ...
        String hqln = "DELETE FROM TblSoccerEmailCron tclnc "
    ...
    public void deleteEmailCronMulti(List<Integer> ids) {
        ...
        String hqln = "DELETE FROM TblSoccerEmailCron tblnc "
    ...
    public boolean updateMatchScoreCard(int matchUniqueId, JSONObject scorecard_data) {
        ...
        try {
            if (!scorecard_data.isEmpty()) {
                ...
                String hqln = "UPDATE TblSoccerMatch tsm "
    ...
    public boolean updatePlayerDetail(int playerUniqueId, LinkedHashMap<String, Object> playerDetail) {
        ...
        if (playerDetail != null && !playerDetail.isEmpty()) {
            ...
            try {
                ...
                String hqln = "UPDATE TblSoccerPlayer tcp "
    ...
    public int updateMatchDreamTeamPlayers(int matchUniqueId, List<Integer> playerUniqueIds) {
        ...
        String hqln = "UPDATE TblSoccerMatchPlayer tcmp " + "SET tcmp.dreamTeamPlayer=:dreamTeamPlayer "
    ...
    public int updateMatchReferralDistributed(int uniqueId, String value) {
        ...
        String hqln = "UPDATE TblSoccerMatch tcm " + "SET tcm.isReferralDistributed=:value "
    ...
    public int updateMatchAffiliateDistributed(int uniqueId, String value) {
        ...
        String hqln = "UPDATE TblSoccerMatch tcm " + "SET tcm.isAffiliateDistribute=:value "
                + "WHERE tcm.uniqueId = :uniqueId";
    ...
    public int updateMatchLeaderboardCreated(int uniqueId, String value) {
        ...
        String hqln = "UPDATE TblSoccerMatch tcm " + "SET tcm.isLeaderboardCreated=:value "
                + "WHERE tcm.uniqueId = :uniqueId";
    ...
    public int updateMatchResultCompleted(int uniqueId) {
        ...
        String hqln = "UPDATE TblSoccerMatch tcm " + "SET tcm.matchResultRunning=:matchResultRunning "
                + "WHERE tcm.uniqueId = :uniqueId";
    ...
    public int updateMatchContestAbondant(int matchUniqueId, String value) {
        ...
        String hqln = "UPDATE TblSoccerMatch tcm " + "SET tcm.isContestAbondantComplete=:value "
                + "WHERE tcm.uniqueId = :uniqueId";
    ...
    public int updateMatchPointsUpdatedAt(int matchUniqueId, String matchProgress, String matchCompletedStatus) {
        ...
        String hqln = "UPDATE TblSoccerMatch tcm " + "SET tcm.pointsUpdatedAt=:value ";
    ...
    public List<HashMap<String, Object>> getMatchContestForAbondant(int matchUniqueId) {
        ...
        String hqln = "SELECT "
                ...
                + "(SELECT COUNT(DISTINCT playerUniqueIdsMultiplers) FROM TblSoccerCustomerTeam where id IN(SELECT customerTeamId FROM TblSoccerCustomerContest WHERE matchContestId=tccm.id)) as clashcount "
    ...
    public HashMap<String, Object> getMatchContestAdminAbondant(int matchContestId) {
        ...
        String hqln = "SELECT "
                + "tccm.id as id, tccm.categoryId as categoryId, tccm.contestJson as contestJson, tccm.totalTeam as totalTeam, tccm.totalJoinedTeam as totalJoinedTeam, tccm.confirmWin as confirmWin, tccm.totalPrice as totalPrice, tccm.actualEntryFees as actualEntryFees, tccm.entryFees as entryFees, tccm.confirmWinContestPercentage as confirmWinContestPercentage, tccm.seriesId as seriesId, tccm.matchUniqueId as matchUniqueId, tccm.isCompressionAllow as isCompressionAllow, tccm.isAbondant as isAbondant " + "FROM TblSoccerContestMatch tccm "
    ...
    public HashMap<String, Object> abodentContest(HashMap<String, Object> contest) {
        ...
        try {
            ...
            String hqln = "SELECT "
                    ...
                    + "FROM TblSoccerCustomerContest tccc " + "WHERE tccc.matchContestId=:matchContestId";
            ...
            if (result.size() == 0) {
                hqln = "UPDATE TblSoccerContestMatch tccm " + "SET tccm.isAbondant='Y' " + "WHERE tccm.id = :id";
                ...
                for (HashMap<String, Object> hashMap : result) {
                    ...
                    if (refundAmount > 0) {
                        hqln = "UPDATE TblSoccerCustomerContest tccc " + "SET tccc.refundAmount=:refundAmount "
                                + "WHERE tccc.id = :id";
                ...
                hqln = "UPDATE TblSoccerCustomerContest tccc " + "SET tccc.isAbondant='Y' "
        ...
        if (!output.isEmpty()) {
            ...
            try {
                ...
                for (Float amount : keySet2) {
                    ...
                    newSession2.save(tblSoccerLineupNotiCron);
    ...
    public HashMap<String, Object> compressContest(HashMap<String, Object> contest, boolean forTesting) {
        ...
        try {
            ...
            if (!forTesting) {
                ...
                String hqln = "UPDATE TblSoccerContestMatch tccm "
    ...
    public HashMap<String, Object> saveResultForContest(HashMap<String, Object> contest,
        ...
        try {
            ...
            for (Integer rank : keySet) {
                ...
                String hqln1 = "UPDATE TblSoccerCustomerContest tccc "
            ...
            hqln = "UPDATE TblSoccerContestMatch tccm " + "SET tccm.isResult='Y' " + "WHERE tccm.id = :id";
        ...
        if (!output.isEmpty()) {
            ...
            try {
                ...
                for (Integer rank : keySet2) {
                    ...
                    tblSoccerLineupNotiCron.setMessage(data.toString());
                    tblSoccerLineupNotiCron.setAlertMessage(alertMessage);
                    tblSoccerLineupNotiCron.setUserIds(customerIdsString);
                    tblSoccerLineupNotiCron.setNotiType(noti_type);
                    tblSoccerLineupNotiCron.setCreated(BigInteger.valueOf(currentTime));
                    ...
                    if (winningTamplate != null) {
                        ...
                        for (Integer customer : customerIds) {
                            ...
                            if (customersData.containsKey(customer)) {
                                ...
                                tblSoccerEmailCron.setToemail(email);
                                tblSoccerEmailCron.setToname(fullName);
                                tblSoccerEmailCron.setEmailType("WIN_CONTEST");

                                newSession2.save(tblSoccerEmailCron);
    ...
    public HashMap<String, Object> saveResultForSeriesLeaderboard(TblSoccerSeries tblSoccerSeries,
                                                                  HashMap<String, Object> totalTaxPercent, TblTemplate winningTamplate) {
        String prizeBrackupData = tblSoccerSeries.getPrizeJson();
        JSONObject prizeBrackup = new JSONObject(prizeBrackupData);
        JSONArray minRanks = prizeBrackup.getJSONArray("per_min_p");
        JSONArray maxRanks = prizeBrackup.getJSONArray("per_max_p");
        JSONArray rankPrice = prizeBrackup.getJSONArray("per_price");
        ...
        int seriesId = tblSoccerSeries.getId();
        ...
        try {
            ...
            Query queryy = newSession.createQuery(hqln).setParameter("seriesId", tblSoccerSeries.getId())
                    .setParameter("lastPosition", lastPosition);
            ...
            for (Integer rank : keySet) {
                ...
                String hqln1 = "UPDATE TblSoccerLeaderboardSeries tcls "
                ...
                Query queryy1 = newSession.createQuery(hqln1).setParameter("winAmount", update_amount)
                        ...
                        .setParameter("totalGadgets", totalGadgets)
                        .setParameter("newRank", rank)
                        .setParameter("seriesId", tblSoccerSeries.getId());
            ...
            hqln = "UPDATE TblSoccerSeries tcs "
                    + "SET tcs.isResultDeclared='Y' "
                    + "WHERE tcs.id = :id";

            queryy = newSession.createQuery(hqln).setParameter("id", tblSoccerSeries.getId());
            ...
            try {
                ...
                for (Integer rank : keySet2) {
                    ...
                    if (winningAmount > 0) {
                        ...
                        alertMessage = "Congratulations! You have won " + GlobalConstant.CURRENCY_SYMBOL
                                + winningAmount + ga_des + "  in the leaderboard " + tblSoccerSeries.getName();
                        ...
                        if (taxAmount > 0) {
                            alertMessage = "Congratulations! You have won " + GlobalConstant.CURRENCY_SYMBOL + winningAmount
                                    ...
                                    + tblSoccerSeries.getName() + " TNC apply.";
                        }

                    } else if (!Util.isEmpty(totalGadgets)) {
                        alertMessage = "Congratulations! You have won "
                                + totalGadgets + "  in the leaderboard " + tblSoccerSeries.getName();
                    ...
                    if (winningTamplate != null) {
                        ...
                        for (Integer customer : customerIds) {
                            ...
                            if (customersData.containsKey(customer)) {
                                ...
                                tblSoccerEmailCron.setToemail(email);
                                tblSoccerEmailCron.setToname(fullName);
                                tblSoccerEmailCron.setEmailType("WIN_LEADERBOARD");
    ...
    public boolean saveOrUpdatePlayerStats(int matchUniqueId, JSONObject players,
                                           LinkedHashMap<String, TblSoccerMatchPlayersStat> playersStatsForMatchUniqueId,
        ...
        try {
            ...
            for (String playerId : keySet) {
                if (playersForMatchUniqueId.has(playerId)) {
                    ...
                    TblSoccerMatchPlayersStat tblSoccerMatchPlayersStat = null;
                    boolean needUpdateInsert = false;
                    if (playersStatsForMatchUniqueId.containsKey(playerId)) {
                        tblSoccerMatchPlayersStat = playersStatsForMatchUniqueId.get(playerId);
                        boolean checkSameStats = tblSoccerMatchPlayersStat.checkSameStats(playerData);
                        if (!checkSameStats) {
                            needUpdateInsert = true;
                            tblSoccerMatchPlayersStat.updateStatsData(playerData);
                            tblSoccerMatchPlayersStat.setUpdated(BigInteger.valueOf(currentTime));
                        }
                    } else {
                        needUpdateInsert = true;
                        tblSoccerMatchPlayersStat = new TblSoccerMatchPlayersStat();
                        tblSoccerMatchPlayersStat.setId(0);
                        tblSoccerMatchPlayersStat.setMatchUniqueId(matchUniqueId);
                        tblSoccerMatchPlayersStat.setPlayerUniqueId(Integer.parseInt(playerId));
                        tblSoccerMatchPlayersStat.setUpdated(BigInteger.valueOf(currentTime));
                        tblSoccerMatchPlayersStat.updateStatsData(playerData);
                    }

                    if (needUpdateInsert) {
                        if (tblSoccerMatchPlayersStat.getId() == 0) {
                            newSession.save(tblSoccerMatchPlayersStat);
                        } else {
                            newSession.update(tblSoccerMatchPlayersStat);
                        }

                        needUpdateRanking = true;

                        String hqlll = "UPDATE " + "TblSoccerMatchPlayer " + "SET points=:totalPoints " + "where "
            ...
            if (!needRemovePlayers.isEmpty()) {
                String hqlll = "DELETE FROM " + "TblSoccerMatchPlayersStat " + "WHERE "
                ...
                float points = 0;
                hqlll = "UPDATE " + "TblSoccerMatchPlayer " + "SET points=:points " + "where "
                        + "matchUniqueId=:matchUniqueId AND playerUniqueId IN(:playerUniqueId)";
        ...
        if (result.size() > 0) {
            ...
            try {
                ...
                for (int i = 0; i < customerTeams.size(); i++) {
                    ...
                    String hqlll = "UPDATE " + "TblSoccerCustomerContest "
    ...
    public boolean startReferralDistributeForMatch(TblSoccerMatch matchDataOnly,
        ...
        String hqln = "SELECT "
                ...
                + "FROM TblSoccerCustomerContest tccc "
                + "LEFT JOIN TblSoccerContestMatch tccm ON (tccc.matchContestId = tccm.id) "
    ...
    public boolean startAffiliateDistributeForMatch(TblSoccerMatch matchDataOnly) {
        boolean isAllDistributed = true;
        int matchUniqueId = matchDataOnly.getUniqueId();
        Session newSession = getNewSession();
        Transaction tx = null;
        try {
            ...
            for (HashMap<String, Object> hashMap : result) {
                ...
                if (affiliatePercent == 0) {
                    ...
                    String hqlll = "UPDATE " + "TblSoccerAffiliate "
                ...
                String hqln2 = "SELECT "
                        ...
                        + "FROM TblSoccerCustomerContest tccc "
                        + "LEFT JOIN TblSoccerContestMatch tccm ON tccm.id=tccc.matchContestId "
    ...
    public boolean startCustomerAffiliateDistributeForMatch(TblSoccerMatch matchDataOnly) {
        boolean isAllDistributed = true;
        ...
        try {
            ...
            String hqln = "SELECT "
                    + "tca.id as id, tca.affiliatePercent as affiliatePercent, tca.customerId as customerId "
                    + "FROM TblSoccerCustomerAffiliate tca "
                    + "WHERE tca.isAffiliateDistribute = 'N' AND tca.matchUniqueId=:matchUniqueId "
                    + "ORDER BY tca.id ASC";
            ...
            for (HashMap<String, Object> hashMap : result) {
                ...
                if (affiliatePercent == 0) {
                    String isAffiliateDistribute = "Y";
                    String hqlll = "UPDATE " + "TblSoccerCustomerAffiliate "
                            + "SET isAffiliateDistribute=:isAffiliateDistribute " + "WHERE " + "id=:id";
                    Query queryyyy = newSession.createQuery(hqlll)
                            .setParameter("isAffiliateDistribute", isAffiliateDistribute)
                            .setParameter("id", affiliate_id);
    ...
    public boolean startLeaderboardCreatingForMatch(TblSoccerMatch matchDataOnly) {
        boolean isAllDistributed = true;
...
//                + "FROM TblSoccerSeriesLeaderboard tcsl "
...
//        }
        ...
        try {
            ...
            String mysqlQuery = "INSERT " + "INTO tbl_soccer_leaderboard_matches "
            ...
            hqln = "SELECT "
                    ...
                    + "FROM TblSoccerLeaderboardSeries tcls " + "WHERE tcls.seriesId = :seriesId ";
            ...
            hqln = "SELECT "
                    + "tcls.id as id, tcls.customerId as customerId, tcls.newPoint as newPoint, tcls.oldPoint as oldPoint, tcls.newRank as newRank, tcls.oldRank as oldRank "
                    + "FROM TblSoccerLeaderboardSeriesWeek tcls "
            ...
            for (JSONObject jsonObject : customerSeriesData) {
                ...
                if (!jsonObject.has("id")) {
                    ...
                    String hqlll = "UPDATE " + "TblSoccerLeaderboardSeries "
            ...
            if (!paramsSeries.isEmpty()) {
                String mysqlQuery1 = "INSERT " + "INTO tbl_soccer_leaderboard_series "
            ...
            for (JSONObject jsonObject : customerSeriesDataWeek) {
                ...
                if (!jsonObject.has("id")) {
                    ...
                    String hqlll = "UPDATE " + "TblSoccerLeaderboardSeriesWeek "
                            + "SET oldRank=:oldRank, newRank=:newRank, oldPoint=:oldPoints, newPoint=:newPoints, updatedAt=:updatedAt  "
                            + "WHERE " + "id=:id";
                    Query queryyyy = newSession.createQuery(hqlll).setParameter("oldRank", oldRank)
            ...
            if (!paramsSeriesWeek.isEmpty()) {
                String mysqlQuery1 = "INSERT " + "INTO tbl_soccer_leaderboard_series_week "
    ...
    public void saveOrUpdateAffiliateContest(Session newSession, int contestJoinedTeam, float contestdeposite_wallet,
        ...
        String hqln = "SELECT " + "tcac.id as id " + "FROM TblSoccerAffiliatesContest tcac "
        ...
        if (result.size() == 0) {
            TblSoccerAffiliatesContest tblSoccerAffiliatesContest = new TblSoccerAffiliatesContest();
            tblSoccerAffiliatesContest.setAdminCommissionAmount(contest_admin_commission_amount);
            tblSoccerAffiliatesContest.setAffiliatePercent(affiliate_percent);
            tblSoccerAffiliatesContest.setBonusAmount(contestcash_bonus_wallet);
            tblSoccerAffiliatesContest.setCreatedAt(currentTime.intValue());
            tblSoccerAffiliatesContest.setCustomerId(customerId);
            tblSoccerAffiliatesContest.setDepositAmount(contestdeposite_wallet);
            tblSoccerAffiliatesContest.setEarning(contest_earning);
            tblSoccerAffiliatesContest.setGivenByAdminAmount(given_by_admin_amount);
            tblSoccerAffiliatesContest.setGivenByUserAmount(given_by_user_amount);
            tblSoccerAffiliatesContest.setJoinedTeam(contestJoinedTeam);
            tblSoccerAffiliatesContest.setMatchContestId(matchContestId);
            tblSoccerAffiliatesContest.setMatchUniqueId(matchUniqueId);
            tblSoccerAffiliatesContest.setTotalWinContribute(total_given_by_admin_and_user_amount);
            tblSoccerAffiliatesContest.setWinningAmount(contestwinning_wallet);

            newSession.save(tblSoccerAffiliatesContest);

        } else {
            HashMap<String, Object> hashMap = result.get(0);
            int rowId = (int) hashMap.get("id");

            String hqlll = "UPDATE " + "TblSoccerAffiliatesContest "
    ...
    public void saveOrUpdateCustomerAffiliateContest(Session newSession, int contestJoinedTeam,
        ...
        String hqln = "SELECT " + "tcac.id as id " + "FROM TblSoccerCustomerAffiliatesContest tcac "
                + "WHERE tcac.matchUniqueId=:matchUniqueId AND tcac.matchContestId=:matchContestId AND tcac.customerId=:customerId ";
        ...
        if (result.size() == 0) {
            TblSoccerCustomerAffiliatesContest tblSoccerAffiliatesContest = new TblSoccerCustomerAffiliatesContest();
            tblSoccerAffiliatesContest.setAdminCommissionAmount(contest_admin_commission_amount);
            tblSoccerAffiliatesContest.setAffiliatePercent(affiliate_percent);
            tblSoccerAffiliatesContest.setBonusAmount(contestcash_bonus_wallet);
            tblSoccerAffiliatesContest.setCreatedAt(currentTime.intValue());
            ...
            String hqlll = "UPDATE " + "TblSoccerCustomerAffiliatesContest "
                    + "SET adminCommissionAmount=:adminCommissionAmount, affiliatePercent=:affiliatePercent, bonusAmount=:bonusAmount, createdAt=:createdAt, depositAmount=:depositAmount, earning=:earning, givenByAdminAmount=:givenByAdminAmount, givenByUserAmount=:givenByUserAmount, joinedTeam=:joinedTeam, totalWinContribute=:totalWinContribute, winningAmount=:winningAmount "
                    + "WHERE " + "id=:rowId";
            Query queryyyy = newSession.createQuery(hqlll)
                    .setParameter("adminCommissionAmount", contest_admin_commission_amount)
    ...
    public void settledAffiliateAmountOnCustomerContest(Session newSession, int matchUniqueId) {
        ...
        for (HashMap<String, Object> contestData : result) {
            ...
            String hqlll = "UPDATE " + "TblSoccerCustomerContest tccc "
            ...
            String hqlll1 = "UPDATE " + "TblSoccerContestMatch "
    ...
    public void updatePlayerPointsInSeries(int seriesId, int gameTypeId) {
        ...
        try {
            ...
            hqln = "SELECT " + "tcsp.id as id, tcsp.playerUniqueId as playerUniqueId "
                    + "FROM TblSoccerSeriesPlayers tcsp "
            ...
            if (result3.size() > 0) {
                for (HashMap<String, Object> hashMap : result3) {
                    ...
                    if (seriesPlayersData.containsKey(playerUniqueId)) {
                        ...
                        String hqlll = "UPDATE " + "TblSoccerSeriesPlayers " + "SET points=:points, updated=:updated "
                                + "WHERE " + "id=:id";
            ...
            if (!seriesPlayersData.isEmpty()) {
                ...
                String mysqlQuery1 = "INSERT " + "INTO tbl_soccer_series_players "
    ...
    public void updateNewAvailableMatchCount(List<LinkedHashMap<String, Object>> newMatchAvailable,
        ...
        try {
            ...
            int soccerSportsId = GlobalConstant.SPORT_ID_SOCCER;
            ...
            Query queryyyy = newSession.createQuery(hqlll).setParameter("newMatchesCount", newMatchesCount)
                    .setParameter("id", soccerSportsId);
            queryyyy.executeUpdate();
            ...
            if (updatedMatchesCount > 0) {
                for (LinkedHashMap<String, Object> match : matchTimeChanged) {
                    ...
                    hqlll = "UPDATE " + "TblSoccerMatch " + "SET matchDate=:matchDate, closeDate=:closeDate " + "WHERE "
                            + "uniqueId=:uniqueId";
    ...
    public void updateCustomerMatchAbondantContestInfo(int matchUniqueId, int matchContestId) {
        ...
        try {
            tx = newSession.beginTransaction();
            String hqln = "SELECT COUNT(DISTINCT tccc.matchContestId) AS cnt, tccc.customerId as customerId FROM TblSoccerCustomerContest tccc WHERE tccc.matchUniqueId =:matchUniqueId AND tccc.isAbondant='Y' GROUP BY tccc.customerId";
            if (matchContestId > 0) {
                hqln = "SELECT COUNT(DISTINCT tccc.matchContestId) AS cnt, tccc.customerId as customerId FROM TblSoccerCustomerContest tccc WHERE tccc.matchUniqueId =:matchUniqueId AND tccc.matchContestId =:matchContestId AND tccc.isAbondant='Y' GROUP BY tccc.customerId";
            }
    ...
    @Transactional(readOnly = true)
    public TblSoccerSeries getSeriesData(int id) {
        String hqlQuery = "SELECT tcs FROM TblSoccerSeries tcs WHERE tcs.status =:status AND tcs.isDeleted =:isDeleted AND tcs.id =:id";
        Query queryObj = getSession().createQuery(hqlQuery);
        queryObj.setParameter("status", "A");
        queryObj.setParameter("isDeleted", "N");
        queryObj.setParameter("id", id);
        return  (TblSoccerSeries) queryObj.uniqueResult();
    }
    ...
    public int updateSeriesLeaderboardResultStart(int id, String result) {
        ...
        String hqln = "UPDATE TblSoccerSeries tcs "
        ...
        try {
            ...
            if (result2 != null && result2.size() > 0) {
                ...
                String updateMatch = "UPDATE "
                        + "TblSoccerMatch "
                        + "SET totalCustomerTeam = (SELECT count(id) FROM TblSoccerCustomerTeam WHERE matchUniqueId=:matchUniqueId) "
                        + "WHERE "
