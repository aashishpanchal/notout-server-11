...
public class ContestRepository {
    ...
    public List<HashMap<String, Object>> getMatchContest(int matchId, int matchUniqueId, Integer UserId) {
        ...
        String hqln = "SELECT "
                ...
                + "FROM  TblCricketContestMatch tccf "
                + "LEFT JOIN TblCricketCustomerContestTeams tccct ON (tccf.id=tccct.matchContestId AND tccct.customerId=:customerId) "
                + "LEFT JOIN TblCricketContestCategory tccc ON (tccc.id=tccf.categoryId ) "
                + "LEFT JOIN TblSetting ts ON(ts.key='DISCOUNTED_IMAGE') "
                + "LEFT JOIN TblCricketCustomerContestFav tcccf ON tcccf.customerId=:customerId AND tcccf.contestId=tccf.contestId "
    ...
    public List<HashMap<String, Object>> getCustomerMatchContest(int matchId, int matchUniqueId, Integer UserId) {
        ...
        String hqln1 = "SELECT " + "tccct.matchContestId as matchContestId from TblCricketCustomerContestTeams tccct "
                + "LEFT JOIN TblCricketContestMatch tccm on tccm.id=tccct.matchContestId"
    ...
    public LinkedHashMap<Integer, Object> getCustomerContestTeams(int customerId, List<Integer> matchContestIds) {
        String hqln = "SELECT  "
                ...
                + "tcct.name as team_name " + "FROM  TblCricketCustomerContest tccc "
    ...
    public List<HashMap<String, Object>> getMatchCategoryContest(int matchId, int matchUniqueId, Integer UserId,
        ...
        String hqln = "SELECT  "
                ...
                + "(CASE WHEN tcccf.id is null THEN 'N' ELSE 'Y' END) as isFavorite "
                + "FROM  TblCricketContestMatch tccf " + "LEFT JOIN TblSetting ts ON(ts.key='DISCOUNTED_IMAGE') "
                + "LEFT JOIN TblCricketCustomerContestTeams tccct ON (tccf.id=tccct.matchContestId AND tccct.customerId=:customerId) "
                + " LEFT JOIN TblCricketContestCategory tccc ON (tccf.categoryId=tccc.id ) "
                + "LEFT JOIN TblCricketCustomerContestFav tcccf ON tcccf.customerId=:customerId AND tcccf.contestId=tccf.contestId "
    ...
    public HashMap<String, Object> getContestWinnerBreakup(int contestId) {
        String hqln = "SELECT id as id, contestJson as contest_json, gadgetDisclaimer as gadget_disclaimer from TblCricketContestMatch where status='A' AND isDeleted='N' AND id=:id";
    ...
    public HashMap<String, Object> getMatchContestShareDetail(int matchContestId) {
        String hqln = "SELECT tccm.slug as slug, tccm.isBeatTheExpert as is_beat_the_expert, tcm.image as image,tcm.name as name from TblCricketContestMatch tccm LEFT JOIN TblCricketMatch tcm ON (tccm.matchId=tcm.id) where (tccm.id=:matchContestId) AND tccm.status='A' AND tccm.isDeleted='N' AND tcm.status='A' AND tcm.isDeleted='N'";
    ...
    public HashMap<String, Object> getMatchContestPdf(int matchContestId) {
        ...
        String hqln = "SELECT COALESCE(pdf,'') as contest_pdf from TblCricketContestMatch where id=:id";
    ...
    public HashMap<String, Object> getBeatTheExpertTeamId(int matchContestId) {
        String hqln = "SELECT teamId as team_id from TblCricketContestMatch where id=:id";
        Query queryy = getSession().createQuery(hqln).setParameter("id", matchContestId);
        List<HashMap<String, Object>> result = (List<HashMap<String, Object>>) queryy
                .setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
    ...
    public int updateBeatTheExpertTeam(int matchContestId, int customerTeamId) {
        String hqln = "UPDATE TblCricketContestMatch tccm set tccm.teamId=:teamId where tccm.id=:id";
    ...
    public List<HashMap<String, Object>> getContestTeams(int userId, int matchUniqueId, int matchContestId, int pageNo,
                                                         TblCricketMatch matchDataOnly, int beatExpertTeamId) {
        ...
        if (pageNo == 1) {
            ...
            String hql = "SELECT "
                    ...
                    + "FROM TblCricketCustomerContest tccc " + "LEFT JOIN TblCustomer tc ON tccc.customerId=tc.id "
                    + "LEFT JOIN TblCricketCustomerTeam tcct ON tcct.id=tccc.customerTeamId "
    ...
    public long getMatchCustomerContestCountByMatchUniqueId(Integer UserId, int matchUniqueId) {
        ...
        String hqln = "SELECT " + "COUNT (DISTINCT tccc.matchContestId ) as id  "
                ...
                + "RIGHT JOIN TblCricketContestMatch tccm ON (tccm.id=tccc.matchContestId) " + "WHERE "
    ...
    public int getContestTeamsCount(int matchUniqueId, int matchContestId) {
        ...
        String hqln = "SELECT " + "count(id) as id " + "from TblCricketCustomerContest tccc " + "where "
    ...
    public HashMap<String, Object> getBeatExpertAdminTeam(int matchUniqueId, int matchContestId) {
        ...
        String hqln = "SELECT " + "tcccat.image as cat_image, "
                ...
                + "from TblCricketCustomerContest tccc " + "LEFT JOIN TblCustomer tc ON(tccc.customerId=tc.id) "
                + "LEFT JOIN TblCricketCustomerTeam tcct ON(tcct.id=tccc.customerTeamId) "
                + "LEFT JOIN TblCricketContestMatch tccm ON(tccm.id=:id) "
                + "LEFT JOIN TblCricketContestCategory tcccat ON(tcccat.id=tccm.categoryId) " + "where "
    ...
    public int ifTeamAlreadyJoinInContest(Integer UserId, Integer[] customerTeamIds, int matchContestId) {
        ...
        String hqln = "SELECT " + "id " + "FROM TblCricketCustomerContest " + "WHERE "
    ...
    public HashMap<String, Object> contestDetailForJoin(Integer UserId, int matchContestId) {
        String hqln = "SELECT "
                ...
                + "LEFT JOIN TblCricketMatch tcm ON(tcm.id=tccm.matchId) "
                + "LEFT JOIN TblCricketContestCategory tccc ON(tccc.id=tccm.categoryId) "
                + "LEFT JOIN TblCricketCustomerContestTeams tccct ON (tccm.id=tccct.matchContestId AND tccct.customerId=:customerId)  "
    ...
    public HashMap<String, Object> contestDetailMini(int matchContestId) {
        String hqln = "SELECT "
                ...
                + "FROM TblCricketContestMatch tccm " + "WHERE "
    ...
    public void createDuplicateMatchContest(int matchContestId) {
        ...
        String hqln = "SELECT "
                ...
                + "FROM TblCricketContestMatch tccm "
                + "LEFT JOIN TblCricketContestMatch tccf1 ON(tccm.parentId=tccf1.id AND tccf1.status='A' AND tccf1.isDeleted='N' AND tccf1.isDuplicateAllow='Y' AND tccf1.isPrivate='N' AND tccf1.isBeatTheExpert='N' AND tccf1.duplicateCount>tccf1.duplicateCreatedCount) "
                + "where "
        ...
        if (result.size() > 0) {
            ...
            if (isDuplicateAllow.equals("Y") && duplicateCount > duplicateCreatedCount
                ...
                String hql = "INSERT INTO " + "TblCricketContestMatch "
                        ...
                        + " FROM TblCricketContestMatch where id=:parentMatchContestId1";
                ...
                String hqllll = "UPDATE " + "TblCricketContestMatch "
                ...
                if (parentMatchContestId != matchContestId) {
                    String hqll = "UPDATE " + "TblCricketContestMatch " + "SET isDuplicatedCreated='Y' " + "where "
                            + "id=:matchContestId";
    ...
    public int selectContestTeamCount(int matchContestId) {
        ...
        String hqln = "SELECT " + "totalJoinedTeam as totalJoinedTeam " + "FROM TblCricketContestMatch " + "WHERE "
                + "id=:matchContestId";
    ...
    public void incrementContestTeamCount(int matchContestId) {
        ...
        String hqln = "UPDATE " + "TblCricketContestMatch " + "SET totalJoinedTeam=totalJoinedTeam+1 " + "WHERE "
                + "id=:matchContestId";
    ...
    public void updateCustomerContestTeams(Integer customerId, int matchUniqueId, int matchContestId) {
        String hqln = "SELECT " + "tcct.id as id, tcct.name as name " + "FROM TblCricketCustomerContest tccc "
                + "LEFT JOIN TblCricketCustomerTeam tcct ON (tcct.id=tccc.customerTeamId) " + "WHERE "
        ...
        hqln = "SELECT " + "tccct.id as id " + "FROM TblCricketCustomerContestTeams tccct " + "WHERE "
        ...
        if (result.size() > 0) {
            ...
            if (rowCount > 0) {
                hqln = "UPDATE " + "TblCricketCustomerContestTeams "
                ...
                TblCricketCustomerContestTeams tblCricketCustomerContestTeams = new TblCricketCustomerContestTeams();
                tblCricketCustomerContestTeams.setMatchContestId(matchContestId);
                tblCricketCustomerContestTeams.setMatchUniqueId(matchUniqueId);
                tblCricketCustomerContestTeams.setCustomerId(customerId);
                tblCricketCustomerContestTeams.setCustomerTeamIds(customerTeamIds);
                tblCricketCustomerContestTeams.setCustomerTeamNames(customerTeamNames);

                getSession().save(tblCricketCustomerContestTeams);
    ...
    public void updateCustomerMatchContestInfo(int matchUniqueId,Integer customerId){
        String hqln = "SELECT COUNT(DISTINCT tccc.matchContestId) AS cnt FROM TblCricketCustomerContest tccc WHERE tccc.matchUniqueId =:matchUniqueId AND tccc.customerId =:customerId";
    ...
    public HashMap<String, Object> getPrivateContestCategory() {
        String hqln = "SELECT "
                ...
                + "FROM " + "TblCricketContestCategory tccc " + "where " + "tccc.isPrivate='Y' AND tccc.isDeleted='N'";
    ...
    @Transactional
    public TblCricketContestMatch createPrivateContest(TblCricketMatch matchDataOnly,
        ...
        try {
            ...
            TblCricketContestMatch tblCricketContestMatch = TblCricketContestMatch.getInstance();

            tblCricketContestMatch.setActualEntryFees(entryFee);
            tblCricketContestMatch.setCategoryId((int) categoryData.get("id"));
            tblCricketContestMatch.setIsCompressionAllow((String) categoryData.get("is_compression_allow"));
            tblCricketContestMatch.setConfirmWin(categoryData.get("confirm_win").toString());
            tblCricketContestMatch.setConfirmWinContestPercentage(
                    Float.parseFloat(categoryData.get("confirm_win_contest_percentage").toString()));
            tblCricketContestMatch.setContestJson(
                    new JSONObject((Map<String, Object>) privateContestBreakupsFromIds.get("contest_json")).toString());
            tblCricketContestMatch.setCreatedAt(BigInteger.valueOf(time));
            tblCricketContestMatch.setEntryFees(entryFee);
            tblCricketContestMatch.setIsPrivate("Y");
            tblCricketContestMatch.setSeriesId(matchDataOnly.getSeriesId());
            tblCricketContestMatch.setMatchId(matchDataOnly.getId());
            tblCricketContestMatch.setMatchUniqueId(matchDataOnly.getUniqueId());
            tblCricketContestMatch.setPerUserTeamAllowed(perUserTeamAllowed);
            tblCricketContestMatch.setTotalJoinedTeam(1);
            tblCricketContestMatch.setTotalPrice(prizePool);
            tblCricketContestMatch.setTotalTeam(contestSize);
            tblCricketContestMatch.setUpdatedAt(BigInteger.valueOf(time));
            tblCricketContestMatch.setUserId(authUserId);

            newSession.save(tblCricketContestMatch);

            int matchContestId = tblCricketContestMatch.getId();

            String slug = Util.getAlphaNumericString(12) + matchContestId + "_";

            tblCricketContestMatch.setSlug(slug);

            String hqln = "UPDATE TblCricketContestMatch tccm set tccm.slug=:slug where tccm.id=:id";
            ...
            TblCricketCustomerContest tblCricketCustomerContest = TblCricketCustomerContest.getInstance();
            tblCricketCustomerContest.setCashBonusWallet(usedBonus);
            tblCricketCustomerContest.setCreated(BigInteger.valueOf(time));
            tblCricketCustomerContest.setCustomerId(authUserId);
            tblCricketCustomerContest.setCustomerTeamId(Integer.parseInt(teamId));
            tblCricketCustomerContest.setDepositeWallet(usedDeposit);
            tblCricketCustomerContest.setEntryFees(entryFee);
            tblCricketCustomerContest.setMatchContestId(matchContestId);
            tblCricketCustomerContest.setSeriesId(matchDataOnly.getSeriesId());
            tblCricketCustomerContest.setMatchUniqueId(matchDataOnly.getUniqueId());
            tblCricketCustomerContest.setUpdated(BigInteger.valueOf(time));
            tblCricketCustomerContest.setWinningWallet(usedWinning);

            newSession.save(tblCricketCustomerContest);

            int lastUserJoinedRowId = tblCricketCustomerContest.getId();
    ...
    @Transactional
    public TblCricketCustomerContest customerJoinContest(TblCricketMatch matchDataOnly, int authUserId,
                                                         int matchContestId, LinkedHashMap<String, Object> extraData) {
        synchronized (lockObject) {
            int totalTeam = (int) extraData.get("totalTeam");
            String hqlnn = "SELECT " + "totalJoinedTeam as totalJoinedTeam " + "FROM TblCricketContestMatch " + "WHERE "
                    + "id=:matchContestId";
            ...
            try {
                ...
                tblCricketCustomerContest.setCreated(BigInteger.valueOf(time));
                tblCricketCustomerContest.setCustomerId(authUserId);
                tblCricketCustomerContest.setCustomerTeamId(teamId);
                tblCricketCustomerContest.setDepositeWallet(usedDeposit);
                tblCricketCustomerContest.setEntryFees(entryFee);
                tblCricketCustomerContest.setMatchContestId(matchContestId);
                tblCricketCustomerContest.setSeriesId(matchDataOnly.getSeriesId());
                ...
                return tblCricketCustomerContest;
    ...
    public int customerSwitchTeam(int customerId, int matchUniqueId, int matchContestId, int customerTeamIdOld,
        ...
        String hql = "UPDATE " + "TblCricketCustomerContest " + "SET customerTeamId=:customerTeamIdNew " + "where "
    ...
    public void saveJoinedContestMail(TblTemplate template, List<HashMap<String, Object>> mailData,
                                      TblCricketMatch matchDataOnly, String toName, String toEmail) {
        ...
        try {
            ...
            for (HashMap<String, Object> hashMap : mailData) {
                ...
                tblCricketEmailCron.setToemail(toEmail);
                tblCricketEmailCron.setToname(toName);
                tblCricketEmailCron.setCreatedat(BigInteger.valueOf(currentTime));
                tblCricketEmailCron.setEmailType("JOIN_CONTEST");

                session.save(tblCricketEmailCron);
            }
        } catch (Exception e) {
        }
    ...
    public BaseResponse updateContestFavorite(Integer UserId, int masterContestId) {
        ...
        String message= "";
        try {
            String hqlQuery = "SELECT tcccf FROM TblCricketCustomerContestFav tcccf WHERE tcccf.customerId =:customerId AND tcccf.contestId =:contestId";
            ...
            TblCricketCustomerContestFav tblCricketCustomerContestFav = (TblCricketCustomerContestFav) queryObj.uniqueResult();
            if (tblCricketCustomerContestFav == null) {
                String mysqlQuery1 = "INSERT " + "INTO tbl_cricket_customer_contest_fav "
                ...
                String hqln = "DELETE FROM TblCricketCustomerContestFav tcccf WHERE tcccf.customerId =:customerId AND tcccf.contestId =:contestId";
